...
private:
    ...
    bool streaming;    // Determines if the output is streamed, or discrete. Streamed mode maintains
                       // ordered, discrete repeats playout most recent data provided.
    ...
    uint8_t bufferPlaying; // ID of the buffer currently being played (0 or 1). Output is hardware
                           // double buffered.
    int8_t stopStreamingAfterBuf; // When stopping, the last buffer ID to play beforhand. -1 if no
                                  // stop is scheduled.
...
public:
    ...
    /**
     * Constructor for an instance of a PWM acting as a sink to a given (likely DMA enabled)
     * datastream. Use this constructor to create a PWM output who's values are controlled
     * dynamically by an asynchronous data stream.
     *
    ...
    /**
     * Defines the mode in which the PWM module will operate, in terms of how it interprets data
     * provided from the DataSource: Valid options are:
     *
     * PWM_DECODER_LOAD_Common          1st half word (16-bit) used in all PWM channels 0..3
     * PWM_DECODER_LOAD_Grouped         1st half word (16-bit) used in channel 0..1; 2nd word in
     * channel 2..3 PWM_DECODER_LOAD_Individual      1st half word (16-bit) in ch.0; 2nd in ch.1;
     * ...; 4th in ch.3 PWM_DECODER_LOAD_WaveForm        1st half word (16-bit) in ch.0; 2nd in
     * ch.1; ...; 4th in COUNTERTOP
     *
    ...
    /**
     * Defines if the PWM module should maintain playout ordering of buffers, or always play the
     * most recent buffer provided.
     *
     * @ param streamingMode If true, buffers will be streamed in order they are received. If false,
     * the most recent buffer supplied always takes prescedence.
     * @ param repeatOnEmpty If set to true, the last buffer received will be repeated when no data
     * is available. If false, the PWM channel will be suspended.
     */
     ...
     */
    int connectPin(Pin &pin, int channel);
